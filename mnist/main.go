...
var model *MNISTNet[*cpu.Backend]
...
func tinygoPredict(inputLen int32, outputLen int32) int32 {
	...
	inputTensor, err := tensor.FromSlice(inputBuf[:], tensor.Shape{784}, backend)
	if err != nil {
	...
	probs := backend.Softmax(logits.Raw(), -1).AsFloat32()
	n := copy(outputBuf[:], probs)
	return int32(n)
}
