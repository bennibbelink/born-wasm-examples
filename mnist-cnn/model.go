...
func (m *MNISTNetCNN[B]) LoadStateDict(stateDict map[string]*tensor.RawTensor) error {
	...
	for k, v := range stateDict {
		if strings.HasPrefix(k, "fc1.") {
			fc1State[k[4:]] = v
		} else if strings.HasPrefix(k, "fc2.") {
			fc2State[k[4:]] = v
		} else if strings.HasPrefix(k, "fc3.") {
			fc3State[k[4:]] = v
		} else if strings.HasPrefix(k, "conv1.") {
			conv1State[k[6:]] = v
		} else if strings.HasPrefix(k, "conv2.") {
			conv2State[k[6:]] = v
		}
	}
